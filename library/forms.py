...
class VisitRecordForm(forms.Form):
    ...
    book = forms.ModelChoiceField(
        ...
        required=False, 
        label='Pilih Buku dari Katalog', 
        empty_label='-- Pilih Buku (Opsional) --', 
        widget=forms.Select(attrs={'class': 'form-select'})
    )
    book_read_manual = forms.CharField(
        max_length=200, 
        label='Atau Tulis Buku Manual', 
        required=False, 
        widget=forms.TextInput(attrs={
            'class': 'form-control',
            'placeholder': 'Tulis judul buku manual jika tidak ada di katalog'
        })
    )

    def clean_book(self):
        """Handle empty or invalid book selection"""
        book = self.cleaned_data.get('book')
        book_raw = self.data.get('book', '').strip()
        
        # Jika value kosong, "Cari buku...", atau string apapun yang bukan ID
        if not book_raw or book_raw == '' or not book_raw.isdigit():
            return None
        
        return book

    def clean_book_read_manual(self):
        """Clean and strip manual book input"""
        book_read_manual = self.cleaned_data.get('book_read_manual', '').strip()
        return book_read_manual
    ...
    def clean(self):
        ...
        if not book and not book_read_manual:
            raise forms.ValidationError(
                'Anda harus memilih buku dari katalog ATAU menulis judul buku manual.'
            )
